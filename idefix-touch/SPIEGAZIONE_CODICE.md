...
Il default della CLI resta `127.0.0.1`, ma il servizio generato dall’installer
passa esplicitamente `--web-bind 0.0.0.0` per permettere l’accesso da telefono
o PC sulla rete di Idefix.
...
L’installer aggiunge `--web-bind 0.0.0.0`: il servizio HTTP ascolta su tutte le
interfacce IPv4, così la pagina è raggiungibile dall’hotspot e dall’Ethernet.
Il browser del touchscreen continua a usare `127.0.0.1:8080`.
